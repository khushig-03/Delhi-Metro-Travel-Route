...
void metro_fare(float dis){
	cout<<"Expected Metro Fare (Weekdays except Saturday and Sunday) would be: Rs. ";
	if(dis<=3.0){ //expected metro fare for shortest route distance <= 2 km.
		cout<<10.0<<endl;
	}
	else if(dis>3.0 && dis<=6.0){
		cout<<20.0<<endl;
	}
	else if(dis>6.0 && dis<=12.0){
		cout<<30.0<<endl;
	}
